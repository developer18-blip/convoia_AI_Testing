        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <StatCard icon={<DollarSign size={20} />} iconColor="#10B981" iconBg="rgba(16,185,129,0.1)"
              value={stats?.thisMonth?.cost ? `$${stats.thisMonth.cost.toFixed(2)}` : '$0'} label="This month" />
            <StatCard icon={<TrendingUp size={20} />} iconColor="#7C3AED" iconBg="rgba(124,58,237,0.1)"
              value={wallet?.totalSpent ? `$${Number(wallet.totalSpent).toFixed(2)}` : '$0'} label="Total spent" />
          </div>
        </div>
      )}
