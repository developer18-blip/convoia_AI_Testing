...
export function AccentProvider({ children }: { children: ReactNode }) {
  // Empty default → getProviderFromModelId('') resolves to 'default' theme
  // (Convoia turquoise). ChatPage/MobileChatPage sync to the active model
  // once a real one is selected; logout clears back to ''.
  const [activeModelId, setActiveModelIdState] = useState<string>('')
  const [councilModels, setCouncilModelsState] = useState<string[]>([])

  // Reset to default Convoia turquoise on logout (dispatched by AuthContext).
  useEffect(() => {
    const handler = () => setActiveModelIdState('')
    window.addEventListener('convoia:logout', handler)
    return () => window.removeEventListener('convoia:logout', handler)
  }, [])
