...
Output ONLY the enhanced prompt — no explanations, no markdown, no quotes. Just the cinematic description in one paragraph, under 200 words.`,
    ...
    // ── PLAIN-CHAT TOOL ROUTING — DISABLED 2026-05-13 ───────
    // Approach 1 (19f9ce6) routed tier-1 plain chats through
    // runPlainChatToolLoop → runNativeFunctionCallingLoop to expose
    // execute_python. That loop uses a non-streaming provider call and
    // emits the full response as a single onChunk, breaking token-by-token
    // streaming for every Anthropic / OpenAI / Google plain chat. Disabled
    // until the inner loop is rewritten to consume streaming provider APIs
    // (Day 4+ ticket). Code Interpreter agent and the manual Run panel
    // both use separate paths and remain functional.
